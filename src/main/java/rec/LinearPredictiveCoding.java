 ...
 * <p>
 * Since this algorithm is generally used in an iterative process over small
 * windows of audio data, it's output reuses the same buffers over and over
 * again in order to avoid memory allocation and garbage collection alltogether.
 * It's up to the client to copy the returned data should it want to keep it
 * as-is and not use it in further processing.
 * </p>
 * <p>
...
public class LinearPredictiveCoding {
	 ...
	 * 
	 * @param window
	 *            windowed part of voice sample, must be of the same size as the
	 *            windowSize passed in constructor
	 * @return an array of size 2 containing LPC coefficients in 0 and error
	 *         coefficients in 1
	 */
