...
public class Recognizer<K> {
	 ...
	 * 
	 * @param sampleRate
	 *            the sample rate, at least 8000.0 Hz (preferably higher)
	 */
	public Recognizer(float sampleRate) {
	...
	/**
	 * Constructor taking previously extracted voice prints directly into the
	 * system
	 * 
	 * @param sampleRate
	 *            the sample rate, at least 8000.0 Hz (preferably higher)
	 * @param voicePrintsByUserKey
	 *            a {@code Map} containing user keys and their respective
	 *            {@code VoicePrint}
	 */
	public Recognizer(float sampleRate, Map<K, VoicePrint> voicePrintsByUserKey) {
		this(sampleRate);
	...
	/**
	 * Sets the universal model to be used to calculate likelihood ratios Once
	 * set, further voice print create / merge operations won't modify this
	 * model
	 * 
	 * @param universalModel
	 *            the universal model to set, may not be null
	 */
	...
	/**
	 * Creates a voice print and stores it along with the user key for later
	 * comparison with new samples
	 * <p>
	 * Threading : this method is synchronized to prevent inadvertently erasing
	 * an existing user key
	 * </p>
	 * 
	 * @param userKey
	 *            the user key associated with this voice print
	 * @param voiceSample
	 *            the voice sample, values between -1.0 and 1.0
	 ...
	 * @param userKey
	 *            the user key associated with this voice print
	 * @param voiceSampleFile
	 *            the file containing the voice sample, must have the same
	 *            sample rate as defined in constructor
	 * @return the voice print
	 * @throws UnsupportedAudioFileException
	 *             when the JVM does not support the file format
	 * @throws IOException
	...
	/**
	 * Converts the given audio file to an array of doubles with values between
	 * -1.0 and 1.0
	 * 
	 * @param voiceSampleFile
	 *            the file to convert
	 ...
	 *             when an I/O exception occurs
	 */
	private double[] convertFileToDoubleArray(File voiceSampleFile) throws UnsupportedAudioFileException, IOException {
	...
	/**
	 * Extracts voice features from the given voice sample and merges them with
	 * previous voice print extracted for this user key
	 * <p>
	 * Threading : it is safe to simultaneously add voice samples for a single
	 * userKey from multiple threads
	 * </p>
	 * 
	 * @param userKey
	 *            the user key associated with this voice print
	 * @param voiceSample
	 *            the voice sample to analyze, values between -1.0 and 1.0
	...
	/**
	 * Calculates the distance between this voice sample and the voice prints
	 * previously extracted and returns the closest matches sorted by distance
	 * <p>
	 * Usage of a closed set is assumed : the speaker's voice print was
	 * extracted before and is known to the system. This means you'll always get
	 * MatchResults even if the speaker is absolutely unknown to the system. The
	 * MatchResult class provides a likelihood ratio in order to help
	 * determining the usefulness of the result
	 * </p>
	 * 
	 * @param voiceSample
	 *            the voice sample, values between -1.0 and 1.0
	...
	public List<MatchResult<K>> identify(double[] voiceSample) {
		...
		for (Entry<K, VoicePrint> entry : store.entrySet()) {
			...
			// likelihood : how close is the given voice sample to the current
			// VoicePrint
			// compared to the total distance between the current VoicePrint and
			// the universal model
	 ...
	 * 
	 * @param voiceSampleFile
	 *            the file containing the voice sample
	 * @return a list MatchResults sorted by distance
	 * @throws UnsupportedAudioFileException
	 *             when the JVM does not support the audio file format
	 * @throws IOException
	 *             when an I/O exception occurs
	 * @see Recognito#identify(double[], float)
	 */
	public List<MatchResult<K>> identify(File voiceSampleFile) throws UnsupportedAudioFileException, IOException {
	...
	/**
	 * Removes silence, applies normalization and extracts voice features from
	 * the given sample
	 * 
	 * @param voiceSample
	 *            the voice sample
