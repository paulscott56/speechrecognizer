 ...
 * <p>
 * Constructor computes default window size by calling
 * {@link #getWindowSize(float)}. <br/>
 * 
 * @see {@link #getWindowSize(float)}
 *      </p>
 * @author Amaury Crickx
 * @param <T>
 *            the kind of features to extract, specified by implementing classes
 */
@Component
public abstract class WindowedFeaturesExtractor<T> implements FeaturesExtractor<T> {
	 ...
	 * 
	 * @param sampleRate
	 *            the sample rate of the voice samples, minimum 8000.0
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.recognito.processing.features.FeaturesExtractor#extractFeatures(
	 * double[])
	 */
	public abstract T extractFeatures(double[] voiceSample);

	/**
	 * Called by the constructor of this class. This implementation delegates to
	 * {@link #getClosestPowerOfTwoWindowSize(float, int)} with default
	 * targetSizeInMillis value
	 * <p>
	 * Implementing classes may wish to override this method by delegating with
	 * another target value in millis or implement another logic alltogether
	 * </p>
	 * 
	 * @param sampleRate
	 *            the sample rate in Hz (times per second), minimum 8000.0
	...
	/**
	 * Computes the window size that is both the closest to targetSizeInMillis
	 * and a power of 2
	 * <p>
	 * Note : window size using a power of 2 is required e.g. when using
	 * discrete FFT algorithm
	 * </p>
	 * 
	 * @param sampleRate
	 *            the sample rate in Hz (times per second), minimum 8000.0
	 * @param targetSizeInMillis
	 *            the target size in millis
	 * @return the window size
	 */
