...
/**
 * A voice activity detector attempts to detect presence or abscence of voice in
 * the signal.
 * <p>
 * The technique used here is a simple (but efficient) one based on a
 * characteristic of (white) noise : when applying autocorrelation, the mean
 * value of the computed cofficients gets close to zero. <br/>
 * Voice activity detection has undergone quite a lot of research, best
 * algorithms use several hints before deciding presence or absence of voice.
 * </p>
 * 
 ...
 * @see <a href="http://en.wikipedia.org/wiki/Voice_activity_detection">Voice
 *      activity detection</a>
 * @see <a href=
 *      "http://ieeexplore.ieee.org/xpl/articleDetails.jsp?arnumber=6403507&punumber%3D97">Unsupervised
 *      VAD article on IEEE</a>
...
public class AutocorrellatedVoiceActivityDetector {
	...
	/**
	 * Returns the noise threshold used to determine if a given section is
	 * silence or not
	 * 
	...
	/**
	 * Sets the noise threshold used to determine if a given section is silence
	 * or not
	 * 
	 * @param threshold
	 *            the threshold
	...
	public double[] removeSilence(double[] voiceSample, float sampleRate) {
		...
		// System.out.println((int)((double)silenceCounter / result.length *
		// 100.0d) + "% removed");
	...
	/**
	 * Gets the minimum voice activity length that will be considered by the
	 * remove silence method
	 * 
	 * @param sampleRate
	 *            the sample rate
	 * @return the length
	...
	/**
	 * Applies a linear fade in / out to the given portion of audio (removes
	 * unwanted cracks)
	 * 
	 * @param voiceSample
	 *            the voice sample
	 ...
	 * 
	 * @param result
	 *            the voice activity result
	 * @param minActivityLength
	 *            the minimum length to apply
	 ...
	 * @param result
	 *            the voice activity result
	 * @param minSilenceLength
	 *            the minimum silence length to apply
	 */
